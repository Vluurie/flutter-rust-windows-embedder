...
use crate::software_renderer::overlay::{
    FlutterOverlay, FLUTTER_OVERLAY_RAW_PTR, // Your struct and global ptr
};
...
/// Core init logic; returns an owned, heap-allocated `FlutterOverlay`.
pub fn init_overlay(
    data_dir: Option<PathBuf>,
    device: &ID3D11Device,
    width: u32,
    height: u32,
) -> Box<FlutterOverlay> { // MODIFIED: Returns Box<FlutterOverlay>
    // crate::init_logging(); // Ensure this is called once at application startup
    info!("[InitOverlay] Initializing FlutterOverlay ({}x{}) - Entry", width, height);
    ...
    // Get a raw pointer from the Box. This pointer is stable as long as overlay_box exists.
    let raw_ptr_to_overlay_data: *mut FlutterOverlay = &mut *overlay_box;
    
    // This user_data will be passed to FlutterEngineInitialize (via run_engine).
    // It points to the data within overlay_box.
    let user_data_for_engine_callbacks = raw_ptr_to_overlay_data as *mut c_void; 
    info!("[InitOverlay] User data for engine callbacks (points to overlay_box data): {:?}", user_data_for_engine_callbacks);
    ...
    //    The pointer comes from the data within overlay_box.
    if let Some(ref custom_runners_box_ref) = overlay_box._custom_task_runners_struct {
        ...
        error!("[InitOverlay] CRITICAL: _custom_task_runners_struct is None in overlay_box. Panicking.");
        panic!("Task runner setup failed: _custom_task_runners_struct is None in overlay_box");
    }
    
    // --- Set FLUTTER_OVERLAY_RAW_PTR *before* calling run_engine ---
    // run_engine will call spawn_task_runner, which uses FLUTTER_OVERLAY_RAW_PTR.
    // This pointer must point to the stable memory location of overlay_box's data.
    unsafe {
        assert!(FLUTTER_OVERLAY_RAW_PTR.is_null(), "FLUTTER_OVERLAY_RAW_PTR should be null before being set here");
        FLUTTER_OVERLAY_RAW_PTR = raw_ptr_to_overlay_data;
        info!("[InitOverlay] FLUTTER_OVERLAY_RAW_PTR set to: {:?} (points to overlay_box data)", FLUTTER_OVERLAY_RAW_PTR);
    }
    ...
    let engine_handle = match crate::software_renderer::overlay::engine::run_engine(
        ...
        raw_ptr_to_overlay_data // Pass the raw_ptr so run_engine can set overlay_box.engine
    ) {
        Ok(handle) => {
        ...
        Err(e) => {
            ...
            unsafe { FLUTTER_OVERLAY_RAW_PTR = ptr::null_mut(); } // Clear global pointer on error
            panic!("Engine initialization failed during run_engine: {}", e);
        }
    };
    // After run_engine, overlay_box.engine is set.
    assert_eq!(overlay_box.engine, engine_handle, "Engine handle in overlay_box mismatch after run_engine");
    ...
    // 11) Return the Box<FlutterOverlay>. The caller now owns this heap-allocated data.
    // FLUTTER_OVERLAY_RAW_PTR points to the data within this Box.
    info!("[InitOverlay] Step 11: FlutterOverlay initialization complete. Returning Boxed overlay instance.");
    overlay_box 
}
