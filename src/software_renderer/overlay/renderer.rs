use crate::embedder;
use super::on_present;

/// Build a software‐renderer config with the *present* callback.
pub fn build_software_renderer_config() -> embedder::FlutterRendererConfig {
    let mut sw: embedder::FlutterSoftwareRendererConfig =
        unsafe { std::mem::zeroed() };
    sw.struct_size = std::mem::size_of::<embedder::FlutterSoftwareRendererConfig>();
    sw.surface_present_callback = Some(on_present);

    let mut cfg: embedder::FlutterRendererConfig =
        unsafe { std::mem::zeroed() };
